	...
	function items() {
		...
		if ($act == 'add') {
			...
			if (intval($data['pid']) == 0) {
				$data['pid'] = $album_mod -> add(array('uid' => $_COOKIE['user']['id'], 'add_time' => time(), ));
			}
			...
			$data['add_time'] = time();
			$data['info'] = "添加了一个宝贝到专辑中~<br/>" . "<a href='" . u("item/index", array('id' => $items_id)) . "' target='_blank'>" . "<img src='" . $res['img'] . "'/></a>";
			$user_history_mod -> add($data);

			$this -> ajaxReturn('success');
		} else if ($act == 'del') {
			//是否是自己的专辑
			$count = $album_mod -> where('id=' . intval($_REQUEST['pid']) . ' and uid=' . $_COOKIE['user']['id']) -> count();
			if ($count == 0)
				return;
