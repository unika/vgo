...
class TaologinAction extends BaseAction {
	function index() {
		...
		$redirect_uri = $this -> site_root . "index.php?m=taologin&state=1";
		...
		//请求参数
		$postfields = array('grant_type' => $grant_type, 'client_id' => $client_id, 'client_secret' => $client_secret, 'code' => $code, 'redirect_uri' => $redirect_uri);
		...
		if ($is_new) {
			...
			cookie('user[login_time]', $last_time, 3600 * 24 * 7);
			cookie('user[key]', $key, 3600 * 24 * 7);
			$data = array('type' => 'tao', 'uid' => $last_uid, 'uname' => $data['name'], 'openid' => $openid, 'info' => $tao_info, );
			//增加加user_info表
