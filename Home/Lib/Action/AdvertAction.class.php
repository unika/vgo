...
class AdvertAction extends BaseAction {
	public function index() {
		...
		$board_type_info =
		include_once (ROOT_PATH . '/data/adboard/' . $board_info['type'] . '.config.php');
		//读取版位下的广告
		if ($board_type_info['option']) {
			...
			if (!$ad) {
				$ad = array('id' => '0', 'type' => 'image', 'code' => 'none.jpg', );
			}
			// print_r($ad);
			$this -> assign('ad', $ad);
		}
		...
		print_r($this -> fetch(ROOT_PATH . '/data/adboard/' . $board_info['type'] . '.html'));
		exit ;
	}
