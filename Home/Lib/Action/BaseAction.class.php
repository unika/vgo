...
class BaseAction extends Action {
	...
	public function _initialize() {
		if (isset($_GET))
			$_GET = setHtmlspecialchars(setFormString($_GET));
		...
		//关注我们
		$follow_us = array('weibo_url' => $this -> setting['weibo_url'], 'qqweibo_url' => $this -> setting['qqweibo_url'], 'renren_url' => $this -> setting['renren_url'], '163_url' => $this -> setting['163_url'], 'qqzone_url' => $this -> setting['qqzone_url'], 'douban_url' => $this -> setting['douban_url'], );
		...
		//我是否关注了ta
		$this -> u['is_follow'] = $this -> user_follow_mod -> where('fans_id=' . $this -> uid . ' and uid=' . $_COOKIE['user']['id']) -> count() > 0;
		//print_r($this->u);exit;
	...
	function get_group_items($cid) {
		$items_mod = M('items');
		$items_result = $items_mod -> field('id,title,simg,bimg') -> where("cid=" . $cid) -> limit("0,6") -> order("is_index DESC,likes DESC") -> select();
		$rel = array();
	...
	protected function check_login() {
		...
		if ($this -> setting['ucenterlogin']) {
			...
			if (!empty($_COOKIE['Ucenter_auth'])) {
				...
				$user = $this -> user_mod -> where("name='" . $uc_username . "'") -> find();
				//获取用户信息 同步discuz那别登录过来的用户
				if ($user) {//生成本系统的用户信息
	...
	function js_init() {
		...
		return json_encode(array("app" => __APP__, "root" => "http://" . $_SERVER['SERVER_NAME'] . $port . __ROOT__ . "/", "user_id" => $_COOKIE['user']['id'], "uid" => $this -> uid, "module" => MODULE_NAME, "action" => ACTION_NAME, "tmpl" => "http://" . $_SERVER['SERVER_NAME'] . $port . __TMPL__, "waterfall_sp" => $this -> setting['waterfall_sp'], "comment_time" => $this -> setting['comment_time'], "masonry" => $this -> setting['show_masonry'], "local_images" => $this -> setting['goods_save_images'], ));
	}
	...
	function get_album_list($where, $img_num = 9) {
		...
		if (S('basic_album_list' . $cid)) {
			...
			$res = $album_mod -> where($where) -> limit($pager -> firstRow . ',' . $pager -> listRows) -> order("sort_order desc,add_time desc") -> select();
			foreach ($res as $key => $val) {
				...
				foreach ($res2 as $key2 => $val2) {
					$img = $items_mod -> field("likes,simg,bimg") -> where("id=" . $val2['items_id']) -> find();
					if ($key2 == 0) {
	...
	function update_user_assoc_num($table, $field = null) {
		...
		$data = array($field . '_num' => $mod -> where('uid=' . $_COOKIE['user']['id']) -> count(), );
	...
	public function sendMail($address, $title, $message) {
		...
		$mail -> From = $this -> setting['mail_username'];
		// 设置邮件头的From字段。
		...
		$mail -> SMTPAuth = true;
		// 设置为“需要验证”
		$mail -> Username = $this -> setting['mail_username'];
		// 设置用户名和密码。
	...
	public function miao_client() {
		define('API_CACHETIME', '24');
		//缓存时间默认为小时   0表示不缓存
		define('API_CACHEPATH', 'Apicache');
		//缓存目录
		define('CHARSET', 'UTF-8');
		//编码
		define('APIURL', 'http://api.59miao.com/Router/Rest?');
		//请求地址http://gw.api.59miao.com/Router/Rest
