...
class UcAction extends BaseAction {
	...
	function index() {
		...
		foreach ($res as $key => $val) {
			...
			foreach ($res2 as $key2 => $val2) {
				$img = $items_mod -> field("id,title,likes,img,simg,bimg") -> where("id=" . $val2['items_id']) -> find();
				if ($key2 == 0) {
	...
	function me() {
		...
		$res = $user_history_mod -> where($where) -> order('add_time desc') -> limit($pager -> firstRow . "," . $pager -> listRows) -> order("id desc") -> select();
	...
	function album_info() {
		...
		if ($act == 'del') {
			...
			$res = $album_mod -> where('id=' . $id . ' and uid=' . $_COOKIE['user']['id']) -> find();
	...
	function share() {
		...
		if ($act == 'del') {
			...
			$data['title'] = ReplaceKeywords(strip_tags($data['title']));
			//替换屏蔽的关键词
			...
			$item_user_data = array('iid' => substr($data['item_key'], 5), 'uid' => $data['uid'], 'item_id' => $new_item_id, 'add_time' => time());
			...
			$data['uname'] = $_COOKIE['user']['name'];
			$data['add_time'] = time();
			$data['info'] = "分享了了一个宝贝~<br/>" . "<a href='" . u("item/index", array('id' => $new_item_id)) . "' target='_blank'>" . "<img src='" . $res['img'] . "'/></a>";
	...
	function nocid_share() {
		...
		if ($affect_row) {
			$item_user_data = array('iid' => substr($data['item_key'], 5), 'uid' => $_COOKIE['user']['id'], 'item_id' => $data['id'], 'add_time' => time());
	...
	private function tixian() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$log_rel = $cash_log -> where("uid='{$uid}' AND after_money!=''") -> order("id desc") -> find();
			//获取用户收支记录
			...
			if (!is_numeric($data['money']) || $data['money'] < $this -> setting["lowest_get_cash"]) {
				...
				if ($user_tixian -> add($data)) {
					//更新用户资金表
					$this -> user_info -> where("uid=$uid") -> setDec('money', $data['money']);
					// 减少用户金钱
					//更新返现记录表
					$cash_back_log = $cash_log;
					$last_info = $cash_back_log -> where("uid='{$uid}'") -> order('id desc') -> limit(1) -> find();
					$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_money' => $last_info['after_money'], 'after_money' => $last_info['after_money'] - $data['money'], 'in_money' => 0, 'out_money' => $data['money'], 'after_jifenbao' => $last_info['after_jifenbao'], 'type' => 2, //1表示收入，2表示支出
					'time' => time(), 'info' => '提现支出', 'sign' => md5($uid . $user_rel['name'] . time()), );
	...
	private function tijifenbao() {
		...
		if (isset($_POST['dosubmit'])) {
			$user_rel = $this -> user_mod -> where("id='{$uid}'") -> relation('user_info') -> find();
			$log_rel = $cash_log -> where("uid='{$uid}' AND after_jifenbao!=''") -> order("id desc") -> find();
			//获取用户收支记录
			$user_tixian = D('user_tixian');
			$data = $user_tixian -> create();
			if (!is_numeric($data['jifenbao']) || $data['jifenbao'] < $this -> setting["lowest_get_jifen_cash"]) {
				...
				if ($user_tixian -> add($data)) {
					//更新用户资金表
					$this -> user_info -> where("uid=$uid") -> setDec('jifenbao', $data['jifenbao']);
					// 减少用户集分宝
					//更新返现记录表
					$cash_back_log = $cash_log;
					$last_info = $cash_back_log -> where("uid='{$uid}'") -> order('id desc') -> limit(1) -> find();
					$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_jifenbao' => $last_info['after_jifenbao'], 'after_jifenbao' => $last_info['after_jifenbao'] - $data['jifenbao'], 'after_money' => $last_info['after_money'], 'in_jifenbao' => 0, 'out_jifenbao' => $data['jifenbao'], 'type' => 2, //1表示收入，2表示支出
					'time' => time(), 'info' => '提现支出', 'sign' => md5($uid . $user_rel['name'] . time()), );
					$cash_back_log -> add($log_data);
					$this -> assign('err', array('err' => 1, 'msg' => '恭喜您提现成功，等待管理员审核'));
				}
	...
	function account_invitation() {
		$this -> assign('share', array('uc_url' => 'http://' . u('uc/index', array('uid' => $_COOKIE['user']['id'])), 'info' => '女人天生爱购物，和我们一起来V购吧！~' . $this -> site_domain));
	...
	function qq_login() {
		...
		$login_url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=" . $this -> setting['qq_app_key'] . "&redirect_uri=" . urlencode($redirect_uri) . "&state=" . $_SESSION['state'];
		header("Location:$login_url");
	}
	...
	function qq_callback() {
		if ($_REQUEST['state'] == $_SESSION['state'])//csrf
		{
			...
			$aGetParam = array("grant_type" => "authorization_code", "client_id" => $this -> setting['qq_app_key'], "client_secret" => $this -> setting['qq_app_Secret'], "code" => $_REQUEST["code"], "redirect_uri" => $this -> site_root . "index.php?m=uc&a=qq_callback");
		...
		if ($is_new) {
			$url = "https://graph.qq.com/user/get_user_info?" . "access_token=" . $_SESSION['access_token'] . "&openid=" . $_SESSION['openid'] . "&oauth_consumer_key=" . $this -> setting['qq_app_key'] . "&format=json";
			$url = "https://graph.qq.com/user/get_user_info";
			$param = array('access_token' => $_SESSION['access_token'], "openid" => $_SESSION['openid'], "oauth_consumer_key" => $this -> setting['qq_app_key'], "format" => 'json', );
			...
			$data = array('name' => $res -> nickname,
			//'img'=>$res->figureurl_2,
			'last_time' => time(), 'last_ip' => $_SERVER['REMOTE_ADDR'], 'add_time' => time(), 'status' => 0, 'ip' => $_SERVER['REMOTE_ADDR'], );
			...
			cookie('user[login_time]', $last_time, 3600 * 24 * 7);
			cookie('user[key]', $key, 3600 * 24 * 7);
			$data = array('type' => 'qq', 'uid' => $last_uid, 'uname' => $data['name'], 'openid' => $_SESSION['openid'], 'info' => serialize($qq_info), );
			//			//保存用户头像
			//
			...
			//	        }
			//增加加user_info表
			$user_info_data = array('uid' => $last_uid, 'info' => '这个人很懒，什么都没有留下');
	...
	function qq_bind() {
		...
		if ($_REQUEST['state'] == $_SESSION['state']) {
			...
			$token_url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&" . "client_id=" . $this -> setting['qq_app_key'] . "&redirect_uri=" . urlencode($this -> site_root . "index.php?m=uc&a=qq_callback") . "&client_secret=" . $this -> setting['qq_app_Secret'] . "&code=" . $_REQUEST["code"];
		...
		if ($user_openid) {
			...
			$data = array('type' => 'qq', 'uid' => $_COOKIE['user']['id'], 'name' => $res -> nickname, 'openid' => $res -> openid, 'info' => serialize($qq_info), );
			$this -> user_openid_mod -> add($data);
		}
	...
	function sina_callback() {
		...
		if (isset($_REQUEST['code'])) {
			...
			if ($token) {
				$c = new SaeTClientV2($this -> setting['sina_app_key'], $this -> setting['sina_app_Secret'], $token['access_token'], '');
		...
		if ($is_new) {
			...
			$data = array('name' => $res['screen_name'],
			//'img'=>$res['profile_image_url'],
			'last_time' => time(), 'last_ip' => $_SERVER['REMOTE_ADDR'], 'add_time' => time(), 'ip' => $_SERVER['REMOTE_ADDR'], 'status' => 0);
			//登录成功
			$last_uid = $this -> user_mod -> add($data);
			$last_time = time();
			$key = md5($last_uid . $data['name'] . $last_time);
			...
			cookie('user[login_time]', $last_time, 3600 * 24 * 7);
			cookie('user[key]', $key, 3600 * 24 * 7);
			$data = array('type' => 'sina', 'uid' => $last_uid, 'uname' => $data['name'], 'openid' => $_SESSION['openid'], 'info' => serialize($sina_info), );
			//增加加user_info表
			$user_info_data = array('uid' => $last_uid, 'info' => '这个人很懒，什么都没有留下');
			$this -> user_info -> add($user_info_data);
			$this -> user_openid_mod -> add($data);
	...
	function sina_bind() {
		...
		if ($user_openid) {
			...
			if ($res) {
				$sina_info = array('user_info' => $res);
				$data = array('type' => 'sina', 'uid' => $_COOKIE['user']['id'], 'uname' => $res['screen_name'], 'openid' => $token['uid'], 'info' => serialize($sina_info), );
				$this -> user_openid_mod -> add($data);
			} else {
				echo '获取用户信息失败!';
				exit ;
		...
		if (isset($_POST['dosubmit']) || $this -> isAjax()) {
			...
			if ($this -> setting['ucenterlogin']) {
				...
				if ($uid > 0) {//ucenter 里存在用户 登录成功
					...
					cookie('user[login_time]', $last_time, 3600 * 24 * 7);
					cookie('user[key]', $key, 3600 * 24 * 7);
					$user_data = array('last_time' => $last_time, 'last_ip' => $_SERVER['REMOTE_ADDR']);
					//更新用户积分
	...
	function logout() {
		...
		if ($_SESSION['login_type'] == 'sina') {
			$url = "https://api.weibo.com/2/account/end_session.json?" . "access_token=" . $_COOKIE['access_token'] . "&source" . $this -> setting['sina_app_key'];
	...
	function sign() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$data = array('name' => trim($_POST['name']), 'passwd' => md5(trim($_POST['passwd'])), 'status' => 1);
			$data_info['integral'] = $this -> setting['user_register_score'];
			//注册成功增加用户积分
	...
	function follow() {
		...
		if ($act == 'add') {
			...
			if (intval($data['fans_id']) == $_COOKIE['user']['id'])
				exit ;
			...
			$user_follow_mod -> where("fans_id=" . intval($_REQUEST['fans_id']) . " and uid=" . $_COOKIE['user']['id']) -> delete();

		}
		if ($act == 'add' || $act == 'del') {
			//更新自己的关注数
			$data = array();
			$data['follow_num'] = $user_follow_mod -> where('uid=' . $_COOKIE['user']['id']) -> count();
		...
		foreach ($res as $key => $val) {
			//我是否关注了ta
			$res[$key]['is_follow'] = $this -> user_follow_mod -> where('fans_id=' . $res[$key]['fans_id'] . ' and uid=' . $_COOKIE['user']['id']) -> count() > 0;
			//该用户最新动态
	...
	function fans() {
		...
		$res = $user_follow_mod -> where($where) -> limit($pager -> firstRow . "," . $pager -> listRows) -> order("id desc") -> select();
		foreach ($res as $key => $val) {
			$res[$key]['is_follow'] = $this -> user_follow_mod -> where('fans_id=' . $res[$key]['uid'] . ' and uid=' . $_COOKIE['user']['id']) -> count() > 0;
			//该用户最新动态
	...
	public function find_password() {
		if (isset($_POST['dosubmit']) && $_POST['find_password'] == 'find_password') {
			...
			if ($user_rel > 0) {
				...
				$add_data = array('uid' => $uid, 'md5' => $md5_data, 'create_time' => $create_time, 'ip' => $ip);
	...
	public function upload() {
		...
		//：缩略图的最大高度，多个使用逗号分隔
		$upload -> thumbFile = "{$_COOKIE['user']['id']}";
		//：指定缩略图的文件名
		...
		//ROOT_PATH."/data/user/{$_COOKIE['user']['id']}/";//：子目录创建方式，默认为hash，可以设置为hash或者date
