...
class IndexAction extends BaseAction {
	...
	function get_index_group_cates() {
		...
		foreach ($index_group_cates as $key => $val) {
			//排序查找子分类
			//二级分类
			$cate2 = $items_cate_mod -> where("pid=" . $val['id'] . " and status=1") -> limit("0,10") -> order("ordid ASC") -> select();
			$ids = "-1";
			foreach ($cate2 as $cate2_key => $cate2_val) {
				$ids .= ',' . $cate2_val['id'];
			}
			//三级分类
			$index_group_cates[$key]['s'] = $items_cate_mod -> where("pid in(" . $ids . ") and status=1") -> limit("0,10") -> order("ordid ASC") -> select();
			//print_r($items_cate_mod->getLastSql());exit;
			//查找需要首页显示的子分类
			$g_result = $items_cate_mod -> where("pid in(" . $ids . ") AND is_hots=1 and status=1") -> order("ordid ASC") -> select();
	...
	private function lately_like() {
		...
		foreach ($like_list as $value) {
			...
			$like_list_array[] = array('title' => $item_info['title'], 'img' => $item_info['img'], 'items_id' => $value['items_id'], 'uid' => $value['uid'], 'time' => $time, 'user_img' => 'data/user/avatar.gif', 'uname' => $user_info['name'], );
		}
