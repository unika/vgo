...
class indexAction extends Action {
	...
	public function show_process() {
		...
		ob_start();
		$temp_info =
		include_once (DATA_PATH . 'install.temp.php');
		...
		$add_time = time();
		$sqls[] = "INSERT INTO `" . $temp_info['db_prefix'] . "admin` (`id`,`user_name`, `password`, `add_time`, `role_id`) VALUES " . "('1','" . $temp_info['admin_user'] . "', '" . $admin_pass . "', '" . $add_time . "', 1);";
		foreach ($sqls as $sql) {
			//替换前缀
			$sql = str_replace('`vg_', '`' . $temp_info['db_prefix'], $sql);
			//获得表名
	...
	private function _set_temp($temp_data) {
		...
		$temp_data = array('db_host' => $temp_data['db_host'], 'db_port' => $temp_data['db_port'], 'db_user' => $temp_data['db_user'], 'db_pass' => $temp_data['db_pass'], 'db_name' => $temp_data['db_name'], 'db_prefix' => $temp_data['db_prefix'], 'admin_user' => $temp_data['admin_user'], 'admin_pass' => $temp_data['admin_pass'], 'admin_email' => $temp_data['admin_email'], );
