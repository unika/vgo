...
class Items_collectAction extends BaseAction {
	...
	public function taobao_batch_collect_jump() {
		...
		if ($key >= count($cate)) {
			...
			$this -> collect_success('第 <em class="blue">' . $index . '</em> 个分类，开始采集下一页', U('items_collect/taobao_batch_collect_jump', array('cate' => implode(',', $cate), 'cate_index' => $index + 1)));
		}
	}
	...
	public function taobao_collect_jump() {
		...
		if ($this -> setting['tao_collect_set'] == 1) {
			$resp = $this -> get_t_item_list($keywords, $p);
			//淘宝商品api采集
		} else {
			$resp = $this -> get_t_c_item_list($keywords, $p);
			//淘宝促销api采集
		}
		$goods_list = (array)$resp -> taobaoke_items;
		$sid = $items_site_mod -> where("alias='taobao'") -> getField('id');
	...
	public function miao_collect_jump() {
		...
		foreach ($goods_list as $item) {
			$item = (array)$item;
			$item['item_key'] = 'miao_' . $item['iid'];
			//用于判断此商品是否存在
			$item['sid'] = $sid;
			$this -> miao_collect_insert($item, $cate_id);
			//数据入库
			$items_nums++;
		}
		//更新分类表商品数
	...
	private function tao_collect_insert($item, $cate_id) {
		...
		if ($isset_item_id) {
			$update_date = array('title' => ReplaceKeywords(strip_tags($item['title'])), 'price' => $item['price'], 'likes' => $item['volume'] + rand(1, 10), 'seller_name' => $item['nick'], 'url' => $item['click_url'], 'cash_back_rate' => round($item['commission'], 2) . '元', 'haves' => $item['volume']);
			$items_mod -> where("id='{$isset_item_id}'") -> save($update_date);

			return;
		}
		$add_time = time();
		$item_id = $items_mod -> add(array('title' => ReplaceKeywords(strip_tags($item['title'])), 'cid' => $cate_id, 'sid' => $item['sid'], 'item_key' => $item['item_key'], 'img' => $item['pic_url'] . '_210x1000.jpg', 'simg' => $item['pic_url'] . '_64x64.jpg', 'bimg' => $item['pic_url'], 'price' => $item['price'], 'url' => $item['click_url'], 'likes' => $item['volume'], 'seller_name' => $item['nick'], 'cash_back_rate' => round($item['commission'], 2) . '元', 'haves' => $item['volume'], 'add_time' => $add_time, ));
		//处理标签
		...
		if (isset($_POST['dosubmit'])) {
			...
			foreach ($goods_list_s as $key => $val) {
				if (in_array($key, $ids_arr)) {
					...
					$val['sid'] = $sid;
					$this -> miao_collect_insert($val, $cate_id);
					//数据入库
					$items_nums++;
				}
			}
	...
	public function catemiao_collect() {
		if (isset($_POST['dosubmit'])) {
			...
			$jump_array = array('cate_id' => $cate_id, 'miaocid' => $miaocid, 'start_price' => $start_price, 'end_price' => $end_price, 'sid' => $sid, 'keyword' => $keyword, 'pages' => $pages);
	...
	public function catemiao_collect_jump() {
		...
		if ($p >= $pages) {
			...
			$jump_array = array('cate_id' => $cate_id, 'miaocid' => $miaocid, 'start_price' => $start_price, 'end_price' => $end_price, 'sid' => $sid, 'keyword' => $keyword, 'pages' => $pages, 'p' => $p + 1);
	...
	public function catetaobao_collect_jump() {
		...
		if ($p >= $pages) {
			...
			$jump_array = array('cate_id' => $cate_id, 'taocid' => $taocid, 'start_price' => $start_price, 'end_price' => $end_price, 'start_commissionRate' => $start_commissionRate, 'end_commissionRate' => $end_commissionRate, 'start_commissionNum' => $start_commissionNum, 'end_commissionNum' => $end_commissionNum, 'start_totalnum' => $start_totalnum, 'end_totalnum' => $end_totalnum, 'levelstart' => $levelstart, 'levelend' => $levelend, 'keyword' => $keyword, 'pages' => $pages, 'p' => $p + 1);
			$this -> collect_success('第 <em class="blue">' . $p . '</em> 页采集完成，开始采集下一页，共 <em class="blue">' . $pages . '</em> 页', U('items_collect/catetaobao_collect_jump', $jump_array));
		}
	}
	...
	function collect_comments() {
		if (isset($_GET['cid']) && isset($_GET['priority'])) {
			...
			//把采集信息写入缓存
			F('cmt_taobao_setting', array('cate_id' => $cate_id, 'order' => $priority . ' DESC', 'users' => $majia, 'pagesize' => $pagesize, 'cmt_num' => $cmt_num, ));
