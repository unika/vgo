...
class Miao_orderAction extends BaseAction {
	...
	public function get_tao_order_jump() {
		...
		if (count($order_report_data) > 0) {
			foreach ($order_report_data as $item) {
				...
				$data = array('uid' => $uid, //暂时放这个             out_code
				'username' => $username, 'order_id' => $item['num_iid'], //59秒开放平台会返回order_id  淘宝没有order_id  用num_iid取代
				'order_code' => $item['trade_id'], 'seller_name' => $item['seller_nick'], 'order_time' => $item['pay_time'], 'item_price' => $item['pay_price'] * $item['item_num'], 'commission' => $item['commission'] * $item['item_num'], 'cash_back_jifenbao' => $cashbac['cacheback'] * $item['item_num'], //返现，这里通过公共函数计算
				'status' => '有效', 'item_count' => $item['item_num'], );
	...
	public function getorder_jump() {
		...
		if (count($order_report_data) > 0) {
			foreach ($order_report_data as $item) {
				...
				$data = array('uid' => $user_info_data['id'], //暂时放这个             out_code
				'username' => $user_info_data['name'], 'order_id' => $item['order_id'], 'order_code' => $item['order_code'], 'seller_name' => $item['seller_name'], 'order_time' => $item['created'], 'item_price' => $item['order_amount'], 'commission' => $item['commission'], 'cash_back' => $cashbac['cacheback'], //返现，这里通过公共函数计算
				'status' => $item['status'], 'item_count' => 1);
	...
	private function update_cash_integral($money, $integral, $uid) {
		...
		if ($user_rel) {//如果此用户存在的话执行操作
			$user_info_mode -> where("uid=$uid") -> setInc('money', $money);
			// 增加用户金钱
			$user_info_mode -> where("uid=$uid") -> setInc('integral', ceil($integral));
			//增加用户积分
			//获取用户信息
			//更新返现记录表
			...
			if (count($last_info) > 0) {//如果存在这条数据的换执行入库操作
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_money' => $last_info['after_money'], 'after_money' => $last_info['after_money'] + $money, 'in_money' => $money, 'out_money' => 0, 'after_jifenbao' => $last_info['after_jifenbao'], 'type' => 1, 'time' => $time, 'info' => '返现收入', 'sign' => md5($uid . $user_rel['name'] . $time), );

			} else {
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_money' => 0, 'after_money' => $money, 'in_money' => $money, 'out_money' => 0, 'after_jifenbao' => 0, 'type' => 1, 'info' => '返现收入', 'time' => $time);
			}
	...
	private function desc_cash_integral($money, $integral, $uid, $info) {
		...
		$user_rel = $user_mode -> where("id=$uid") -> field('id,name') -> find();
		if ($user_rel) {//如果此用户存在的话执行操作
			$user_info_mode -> where("uid=$uid") -> setDec('integral', ceil($integral));
			//增加用户积分
			$user_info_mode -> where("uid=$uid") -> setDec('money', $money);
			// 增加用户金钱
			//获取用户信息
			...
			$last_info = $cash_back_log -> where('uid=' . $uid . '') -> order('id desc') -> limit(1) -> find();
			if (count($last_info) > 0) {//如果存在这条数据的换执行入库操作
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_money' => $last_info['after_money'], 'after_money' => $last_info['after_money'] - $money, 'in_money' => 0, 'out_money' => $money, 'after_jifenbao' => $last_info['after_jifenbao'], 'type' => 2, //1表示收入  2表示支出
				'time' => $time, 'info' => $info . '支出', 'sign' => md5($uid . $user_rel['name'] . $time), );
	...
	private function update_cash_jifenbao_integral($jifenbao, $integral, $uid) {
		...
		if ($user_rel) {//如果此用户存在的话执行操作
			$user_info_mode -> where("uid=$uid") -> setInc('integral', ceil($integral));
			//增加用户积分
			$user_info_mode -> where("uid=$uid") -> setInc('jifenbao', ceil($jifenbao));
			// 增加用户金钱
			//获取用户信息
			...
			$last_info = $cash_back_log -> where("uid='{$uid}'") -> order('id desc') -> limit(1) -> find();
			if (count($last_info) > 0) {//如果存在这条数据的换执行入库操作
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_jifenbao' => $last_info['after_jifenbao'], 'after_jifenbao' => $last_info['after_jifenbao'] + $jifenbao, 'after_money' => $last_info['after_money'], 'in_jifenbao' => $jifenbao, 'out_jifenbao' => 0, 'type' => 1, 'time' => $time, 'info' => '返现收入', 'sign' => md5($uid . $user_rel['name'] . $time), );

			} else {
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_jifenbao' => 0, 'after_jifenbao' => $jifenbao, 'in_jifenbao' => $jifenbao, 'out_jifenbao' => 0, 'after_money' => 0, 'type' => 1, 'info' => '返现收入', 'time' => $time);
			}
			$cash_back_log -> add($log_data);
		}
	}
	...
	private function desc_cash_jifenbao_integral($jifenbao, $integral, $uid, $info) {
		...
		if ($user_rel) {//如果此用户存在的话执行操作
			$user_info_mode -> where("uid=$uid") -> setDec('integral', ceil($integral));
			//增加用户积分
			$user_info_mode -> where("uid=$uid") -> setDec('jifenbao', $jifenbao);
			// 增加用户金钱
			//更新返现记录表
			$cash_back_log = D('cash_back_log');
			$time = time();
			$last_info = $cash_back_log -> where('uid=' . $uid . '') -> order('id desc') -> limit(1) -> find();
			if (count($last_info) > 0) {//如果存在这条数据的换执行入库操作
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_jifenbao' => $last_info['after_jifenbao'], 'after_jifenbao' => $last_info['after_jifenbao'] - $jifenbao, 'in_jifenbao' => 0, 'out_jifenbao' => $jifenbao, 'after_money' => $last_info['after_money'], 'type' => 2, //1表示收入  2表示支出
				'time' => $time, 'info' => $info . '支出', 'sign' => md5($uid . $user_rel['name'] . $time), );
