...
class ItemsAction extends BaseAction {
	...
	function delete() {
		...
		if (isset($_REQUEST['id'])) {
			...
			$items_likes_mod -> where("items_id in(" . $cate_ids . ")") -> delete();
			//用户喜欢表
			$items_tags_item_mod -> where("item_id in(" . $cate_ids . ")") -> delete();
			//商品标签表
			$items_comments_mod -> where("pid in(" . $cate_ids . ")") -> delete();
			//商品品论
			$album_items_mod -> where("items_id in(" . $cate_ids . ")") -> delete();
			//专辑商品表
			$items_user_mod -> where("item_id in(" . $cate_ids . ")") -> delete();
			//删除用户分享表里面的信息
			$items_cate_mod -> upCateNum($_REQUEST['id']);
			//更新items_cate表里面的数据
		}
		$this -> success(L('operation_success'));
	}
	...
	private function tao_collect_insert($item, $cate_id) {
		...
		$item_id = $items_mod -> add(array('title' => strip_tags($item['title']), 'cid' => $cate_id, 'sid' => $item['sid'], 'item_key' => $item['item_key'], 'img' => $item['img'], 'simg' => $item['simg'], 'bimg' => $item['pic_url'], 'price' => $item['price'], 'url' => $item['click_url'], 'likes' => 0, 'haves' => 0, 'add_time' => $add_time, ));
		if ($item_id) {
		...
		if (isset($_REQUEST['dosubmit'])) {
			if ($_REQUEST['isok'] == "1") {
				...
				if ($ids != "") {
					...
					//用户喜欢表
					$items_tags_item_mod -> where("item_id in(" . $ids . ")") -> delete();
					//商品标签表
					$items_comments_mod -> where("pid in(" . $ids . ")") -> delete();
					//商品品论
					$album_items_mod -> where("items_id in(" . $ids . ")") -> delete();
					//专辑商品表
					$items_user_mod -> where("item_id in(" . $ids . ")") -> delete();
					//删除用户分享表里面的信息
