...
class DatabaseAction extends BaseAction {
	...
	public function dumptable() {
		...
		$table_vars = array('count' => count($table_data['tables']), "index" => $index + 1, "name" => $table);
	...
	private function dirFileInfo($dir) {
		...
		while (($file = readdir($dirhandle)) !== false) {
			if (($file != ".") && ($file != "..") && is_dir($dir . $file) && file_exists($dir . $file . '/tables.php')) {
				...
				$list[] = array('filename' => $table_data['file_dir'], 'filetime' => $table_data['time'], 'filedate' => toDate($table_data['time']));
			}
		}
		@closedir($dirhandle);
		usort($list, fileSort);
