	...
	public function back_update() {
		...
		if (false !== $result) {
			...
			if ($type == 2) {//2表示集分宝回滚
				...
				//更新用户资金表
				$this -> user_info -> where("uid=$uid") -> setInc('jifenbao', $jifenbao);
				// 减少用户金钱
				//更新返现记录表
				$cash_back_log = D('cash_back_log');

				$last_info = $cash_back_log -> where("uid='{$uid}'") -> order('id desc') -> limit(1) -> find();
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_jifenbao' => $last_info['after_jifenbao'], 'after_jifenbao' => $last_info['after_jifenbao'] + $jifenbao, 'after_money' => $last_info['after_money'], 'in_jifenbao' => $jifenbao, 'out_jifenbao' => 0, 'type' => 1, //1表示收入，2表示支出
				'time' => time(), 'info' => '提现失败,资金退回', 'sign' => md5($uid . $user_rel['name'] . time()), );
				...
				$user_rel = $this -> user_mode -> where("id='{$uid}'") -> find();
				//更新用户资金表
				$this -> user_info -> where("uid=$uid") -> setInc('money', $money);
				// 减少用户金钱
				//更新返现记录表
				$cash_back_log = D('cash_back_log');

				$last_info = $cash_back_log -> where("uid='{$uid}'") -> order('id desc') -> limit(1) -> find();
				$log_data = array('uid' => $uid, 'uname' => $user_rel['name'], 'before_money' => $last_info['after_money'], 'after_money' => $last_info['after_money'] + $money, 'in_money' => $money, 'out_money' => 0, 'after_jifenbao' => $last_info['after_jifenbao'], 'type' => 1, //1表示收入，2表示支出
				'time' => time(), 'info' => '提现失败,资金退回', 'sign' => md5($uid . $user_rel['name'] . time()), );
				$cash_back_log -> add($log_data);
			}
